...
export function PageHeader({title,description,href,label="신규 등록"}:{title:string;description?:string;href?:Route;label?:string}){return <div className="flex items-start justify-between"><div><h2 className="text-2xl font-bold">{title}</h2>{description&&<p className="mt-1 text-sm text-slate-500">{description}</p>}</div>{href&&<Link className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white" href={href}>{label}</Link>}</div>}
...
export function Empty({text="등록된 데이터가 없습니다."}:{text?:string}){return <div className="py-16 text-center text-sm text-slate-500">{text}</div>}
