...
견적서 상세 또는 목록에서 A4 미리보기와 브라우저의 **PDF로 저장** 인쇄 기능을 사용할 수 있습니다. `/api/quotations/[id]/excel`은 동일한 검증된 출력 데이터를 사용해 실제 `.xlsx` 견적서를 생성합니다. Excel 출력은 `exceljs`를 사용하며, 품목 숫자·날짜 서식과 A4 세로 인쇄 설정을 포함합니다. 직인과 로고를 읽지 못하거나 ExcelJS가 지원하지 않는 BMP인 경우 이미지만 제외하고 문서는 계속 생성됩니다.
